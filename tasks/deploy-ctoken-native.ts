...
/**
 * npx hardhat deploy-ctoken-native \
 * --network blast \
...
task("deploy-ctoken-native", "Deploys a new ctoken")
  ...
  .setAction(async (args, hre, runSuper) => {
    ...
    const contractKey = `CEtherImmutable_${underlyingSymbol}`;
    ...
    try {
      ...
      cToken = await deploy(contractKey, {
        from: deployer,
        log: true,
        contract: "contracts/CEtherImmutable.sol:CEtherImmutable",
        args: [
